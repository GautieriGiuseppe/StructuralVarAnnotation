...
Main chapters:
  ...
  6. needLR annotation and population frequency
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build full GRCh38 cohort QC HTML report."
    )
...
def build_read_qc_discovery(samples_df, workflow_outdir, cohort_results_dir):
    ...
    for _, row in samples_df.iterrows():
        ...
        # New generated read QC outputs live under cohort_results/read_qc/{tool}/{batch}/{sample}
        if os.path.exists(read_qc_root):
            sample_read_qc_dirs = [
                os.path.join(read_qc_root, "fastqc", batch, sample),
...
def render_read_qc_section(
    ...
    if total_files == 0:
        ...
        <div class="missing">
          ...
          <code>cohort_results/read_qc</code>. If FastQC/NanoPlot rules were just
    ...
    if read_qc_images:
        html += "<h3>Representative FastQC/NanoPlot images</h3>"
...
def build_html_report(
    ...
    read_qc_body = f"""
    <p>
      This section summarizes FastQC, NanoPlot, and MultiQC read-level QC
      artifacts. Long file-path lists are hidden to keep the report readable.
    </p>
    ...
    <nav class="toc">
      ...
      <ol>
        ...
        <li><a href="#needlr">needLR annotation and population frequency</a></li>
    ...
    body = "\n".join([
        ...
        section("needLR annotation and population frequency", "needlr", needlr_body),
...
<head>
  <meta charset="utf-8">
  <title>GRCh38 full pipeline QC report</title>
  {css}
</head>
<body>
  <header>
    <h1>GRCh38 full pipeline QC report</h1>
    <p>Integrated native GRCh38 and CHM13-to-GRCh38 structural-variant workflow</p>
  </header>
  <main>
    {toc}
    {body}
